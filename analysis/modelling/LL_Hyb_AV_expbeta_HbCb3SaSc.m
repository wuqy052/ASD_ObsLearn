function [f,vals] = LL_Hyb_AV_expbeta_HbCb3SaSc(params,P)
...
% load('/Users/wuqy0214/Documents/psy/OLab/ObservationalLearning/OL_models_for_Qianying/OL_models_for_Qianying/data_allsubs_Oct2020.mat');
% P_table = data(50).data;
% P = table2array(P_table);
% params = [0.4, 0.6, 1.2, 0.5, 0.6, 0.2, 0.9];
...
w = 1/(1+exp(-params(2)));   % fixed arbitration weight [0 1]
w_hand = params(3); % hand bias weight, no need to transform
w_rb = params(4); % color bias (red vs blue), no need to transform
w_g = params(5); % color bias (green), no need to transform
...
w_stickyact = params(6); % sticky stimulus weight, no need to transform
d_stickycol = 0;
w_stickycol = params(7); % sticky stimulus color, no need to transform
d_stickyact  = 0;
...
for t=1:tr_nb
    ...
    elseif tr_type(t)==2 && (ismiss(t) == 0)%play
        ...
        AVim_c = AVim(t,:);
        AVim_c(UA)=[]; %isolate the probabilities of the 2 available actions
        AVim_diff(t) = AVim_c(1) - AVim_c(2); % store the AVdiff of imitation
       ...
       % hand bias
        ...
        %  calculate joint AV and P_left
        val_diff_all(t) = AVbi_diff(t) * w + AVim_diff(t) * (1-w) + AVbl_diff(t);
...
vals = [LH P_left ll_corr prior_V V AVbi AVim val_diff_all];
end