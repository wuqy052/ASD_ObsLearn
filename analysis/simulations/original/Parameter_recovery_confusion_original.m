...
% alternatives: 1) fit with a subset of actual participant parameters,
% still generate 10 sets of simluations; 2) fit with a fixed beta, only
% nonhbi

%load data
load(['..' fs '..' fs 'modelling' fs 'original_data' fs 'hbi_IntDyn_original.mat']);
% parameters
...
for rep = 1:10 % repeat 10 times
    ...
    for tl=1:10
        for it=1:10 % generate 10 datasets
            ...
            P_new(:,17) = zeros(84,1); % missing trial
...
save('/Users/wuqy0214/Library/CloudStorage/Box-Box/OL_shared/analysis/simulations/original/IntDyn/sim_1000_data_params.mat','params_sim_all','data_sim_all');
...
for it=1:10
    ...
    load(['/Users/wuqy0214/Library/CloudStorage/Box-Box/OL_shared/analysis/simulations/original/IntDyn/',filename]);
...
%xlabel('Simulated parameters','FontSize',10);
caxis([-1,1]);
%title({'Parameter recovery','Discovery sample'});
...
load(['..' fs '..' fs 'modelling' fs 'original_data' fs 'hbi_5mods_original.mat']);
% parameters
...
for mod = 1:5
    ...
    save(['/Users/wuqy0214/Library/CloudStorage/Box-Box/OL_shared/analysis/simulations/original/',modnames{mod},'/sim_1000_data_params.mat'],'params_sim_all','data_sim_all');
...
colorbar; 
colormap(J);
%xlabel('Model used for data generation');
%ylabel('Model used for fitting');
%title({'Exceedance probability','Discovery sample'});
